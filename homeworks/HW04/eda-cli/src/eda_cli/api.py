...
from typing import Dict, Any  # ← ДОБАВЬ ЭТУ СТРОЧКУ
...
# ---------- Новый эндпоинт для полного набора флагов ----------
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> Dict[str, Any]:
    """
    Эндпоинт для получения полного набора флагов качества данных из CSV файла.
    Возвращает ВСЕ флаги (включая новые из HW03) без интегральной оценки.
    """
    start = perf_counter()
    ...
    # ВАЖНО: используем функцию compute_quality_flags, которая уже включает
    # ВСЕ флаги из HW03 (has_constant_columns, has_high_cardinality_categoricals и т.д.)
    flags_all = compute_quality_flags(summary, missing_df)
    
    latency_ms = (perf_counter() - start) * 1000.0

    # Возвращаем только булевы флаги (отфильтровываем числовые значения)
    bool_flags = {}
    for key, value in flags_all.items():
        if isinstance(value, bool):
            bool_flags[key] = value
        elif key == "quality_score":
            # Сохраняем score отдельно, если нужно
            continue
        elif isinstance(value, (int, float)):
            # Можно добавить числовые метрики отдельно, если нужно
            continue

    print(
        f"[quality-flags-from-csv] filename={file.filename!r} "
        f"n_flags={len(bool_flags)} latency_ms={latency_ms:.1f} ms"
    )
