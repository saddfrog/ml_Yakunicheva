...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # НОВАЯ ЭВРИСТИКА 1: есть ли константные колонки
    flags["has_constant_columns"] = any(
        col.unique == 1 and col.missing == 0 for col in summary.columns
    )
    
    # НОВАЯ ЭВРИСТИКА 2: есть ли категории с высокой кардинальностью
    flags["has_high_cardinality_categoricals"] = any(
        not col.is_numeric and col.unique > summary.n_rows * 0.9
        for col in summary.columns
    )

    # НОВАЯ ЭВРИСТИКА 3: много ли нулевых значений в числовых колонках
    # Для простоты считаем, что есть если много пропусков
    flags["has_many_zero_values"] = False  # ← можно сделать свою функцию
    
    # НОВАЯ ЭВРИСТИКА 4: есть ли дубликаты ID
    flags["has_suspicious_id_duplicates"] = False  # ← можно сделать свою функцию
